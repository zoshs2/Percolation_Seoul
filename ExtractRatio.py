def ExtractRatio(date_dataset:pd.DataFrame) -> pd.DataFrame():
    '''
    ...
    LMV = date_dataset.loc[:, ['PRCS_YEAR', 'PRCS_MON', 'PRCS_DAY', 'LINK_ID', 'PRCS_SPD']].groupby(['PRCS_YEAR', 'PRCS_MON', 'PRCS_DAY', 'LINK_ID']).quantile(0.95).rename({'PRCS_SPD':'LMV'}, axis='columns')
    LMV = LMV.reset_index().sort_values(['PRCS_YEAR', 'PRCS_MON', 'PRCS_DAY', 'LINK_ID'], ascending=True)
    date_dataset = date_dataset.sort_values(['PRCS_YEAR', 'PRCS_MON', 'PRCS_DAY', 'LINK_ID'], ascending=True)
    
    one_day = int(date_dataset.shape[0] / LMV.shape[0])
    ...
    for lmv in LMV['LMV'].values:
        rslt = date_dataset[start:start+one_day]['PRCS_SPD'] / lmv
        rslt = rslt.to_list()
        ratio_list.extend(rslt)
        start += one_day
        
    date_dataset['ratio'] = ratio_list
    return date_dataset